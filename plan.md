...
**求值契约**：`evalWhen(expr, ctx) → {ok:true, value:boolean} | {ok:false, reason}`。返回 `ok:false` 时**记为 UNCOVERED 并告警，绝不静默当作 false**。**触发语义（已决，§12.1 #23）**：每根已收盘 bar 求值一次，`fired` 去重键含 `barTs` ⇒ **条件持续为真时每根 bar 都会触发（电平）**。★ **目前无法表达"只在穿越那一次触发"**：`crossAbove`/`crossBelow` 虽然写在本文的函数表里，但**尚未实现**（`defaultFunctions` 刻意不提供、也没有任何调用方注入，见 `AGENTS.md` 诚实清单），写它们会让求值 `ok:false` ⇒ 该条件**永久 UNCOVERED**（fail-closed，不会乱下单，但计划卡永远不会触发）。边沿表达（实现 `cross*`）还是 edge-default（需持久化每条件状态）属未决项，见 §12.2 I。解析与求值实现为零依赖纯函数，单测覆盖每个算子与每个错误分支（P0 门禁：表达式编译成功率 100%）。
...
| I | 表达式层**无法表达边沿**（`cross*` 未实现） | 人工决定方向；实现属 features/evaluator 改动 | `plan §3.2` 的函数表与 `tests/plan-card.test.ts` 的示例卡都写了 `crossBelow(...)`，但 `src/plan/dsl.ts` 的 `defaultFunctions` **刻意不提供**它，`createFeatureContext` 也没有注入——**全仓无实现**。后果：用 `cross*` 的条件的求值 `ok:false` ⇒ **永久 UNCOVERED**（fail-closed 不会乱下单，但计划卡永不触发，且很难发现）。两条路：**(a) 实现 `cross*`**（需要 evaluator 支持"用前一根 bar 的取值重算参数"，即 `DslContext` 增加 previous 取值面 + live-engine/replay 传入前一根快照）；**(b) 改 edge-default**（需持久化每条件 `last_value`，否则重启会重复触发）。**我建议 (a)**：它让词表名副其实，改动局部，且不改变现有电平条件的行为 |
