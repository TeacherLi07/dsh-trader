# P2 阶段验收汇总（真实接口与故障注入，2026-09-15）

> 逐条判据的可运行命令与原始 JSON：
> - ① ② ④ → `docs/p2-fault-injection-2026-09-15.md` / `.json`（`node scripts/fault-injection.mjs`）
> - ③ → `docs/p2-watchdog-2026-09-15.md` / `.json`（`node scripts/watchdog-check.mjs`）
>
...
| ③ | `SIGSTOP` > 3× 心跳 → watchdog 撤单、交易所挂单 = 0 | ✅ 8/8：真实 `SIGSTOP` → stale → 撤单 → 挂单清空 → halted 落库 → 幂等 | 独立进程读同一 WAL 库 |
...
| T2.2 | `supervisor/{heartbeat,watchdog}.ts`、`Reconciler`、`/halt` `/resume`、`scripts/watchdog-check.mjs` | 撤单成功**才**置 halted（否则只告警、下轮重试）；无 heartbeat 行按 stale（fail-closed）；`inject = ['commands']` |
